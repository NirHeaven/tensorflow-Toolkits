...
__all__ = ['sg', 'sg_full']
def sg_full(frame_imgs, trn_Flag, keep_prob=0.5, out_channels=10, return_fea_map=False, name='sg_full'):
...
def sg(frame_imgs, trn_Flag, keep_prob=0.5, out_channels=10, return_fea_map=False, name='sg'):
    # input_layer = tf.reshape(frame_imgs, shape=[-1, target_height, target_width,  img_channel])
    with variable_scope.variable_scope(name):
        conv1 = _conv2d(frame_imgs, 48, 5, 5, 2, 2, name='sg_conv1')
        bn1 = _batch_norm(conv1, trnFlag=trn_Flag, name='sg_conv1_bn')
